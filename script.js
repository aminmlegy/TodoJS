...
const Wrap = (function () {
  ...
  function editItem(val, cond) {
    let itemEdit = { idTodo: cond, todoVal: val };
    arr = arr.map((item) => {
      return item.idTodo === itemEdit.idTodo ? itemEdit : item;
    });
    localStorage.setItem("items", JSON.stringify(arr));
...
Form.addEventListener("submit", (e) => {
  e.preventDefault();
  if (Wrap.condations === "show") {
    let idRandoum = Math.floor(Math.random() * 100000);
    if (inputText.value.trim() !== "") {
      Wrap.add(idRandoum, inputText.value.trim());
    }
  } else if (!Wrap.condations === "show") {
    Wrap.editItem(inputText.value.trim(), Wrap.condations);
  }
  Wrap.showItems();
  inputText.value = "";
});
...
document.addEventListener("click", (e) => {
  if (e.target.className === "delete") {
    ...
    Wrap.getItem(e.target.dataset.edit);
  }
});
